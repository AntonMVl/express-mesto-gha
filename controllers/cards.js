const { HTTP_STATUS_BAD_REQUEST, HTTP_STATUS_INTERNAL_SERVER_ERROR, HTTP_STATUS_OK, HTTP_STATUS_CREATED, HTTP_STATUS_NOT_FOUND, HTTP_STATUS_RESET_CONTENT } = require('http2').constants
...
module.exports.addCardLike = (req, res) => {
  return cardModel.findByIdAndUpdate(
    ...
    .then((r) => { return res.status(HTTP_STATUS_RESET_CONTENT).send(r) })
    .catch((e) => {
      if (e instanceof mongoose.Error.ValidationError) {
        return res.status(HTTP_STATUS_BAD_REQUEST).send({ message: 'Invalid Data' })
      }
